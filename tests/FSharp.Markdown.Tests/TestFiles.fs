...
let rec runTests dir =
    for file in Directory.GetFiles(dir, "*.text") do
        transformAndCompare
            (fun (doc, writer, newline) -> Markdown.WriteHtml(doc, writer, newline))
            file
            (file.Replace(".text", ".2.html"))
            (file.Replace(".text", ".html"))

    for file in Directory.GetFiles(dir, "*.text") do
        transformAndCompare
            (fun (doc, writer, newline) -> Markdown.WriteLatex(doc, writer, newline))
            file
            (file.Replace(".text", ".2.tex"))
            (file.Replace(".text", ".tex"))
