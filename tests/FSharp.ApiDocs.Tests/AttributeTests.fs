...
let ``ApiDocs extracts Attribute on Module``() =
  let modules = ApiDocs.GenerateModel([library], info, libDirs = [testBin]).EntityInfos
...
let ``ApiDocs formats attribute without arguments``() =
  let moduleInfos = ApiDocs.GenerateModel([library], info, libDirs = [testBin]).EntityInfos
