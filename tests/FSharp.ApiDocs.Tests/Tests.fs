...
let generate (libraries:string list) useMarkdown =
    try let output = getOutputDir ()
        ...
        ApiDocs.GenerateFromModelWithDotLiquid (metadata, output, layoutRoots)
...
let ``ApiDocs works on sample FAKE assembly``() =
  ...
  let output = getOutputDir()
  ApiDocs.GenerateWithDotLiquid([library], output, layoutRoots, info)
  let files = Directory.GetFiles(output)
...
let ``ApiDocs works on two sample F# assemblies``() =
  let libraries =
    [ testBin </> "FsLib1.dll"
      testBin </> "FsLib2.dll" ]
  let output = getOutputDir()
  ApiDocs.GenerateWithDotLiquid(libraries, output, layoutRoots, info, libDirs = [testBin])
  let fileNames = Directory.GetFiles(output)
  let files = dict [ for f in fileNames -> Path.GetFileName(f), File.ReadAllText(f) ]
...
let ``ApiDocs process XML comments in two sample F# assemblies``() =
  ...
  //ApiDocs.GenerateFromModelWithDotLiquid(libraries, output, layoutRoots, info, libDirs = [root </> "../../lib"; root </> "../../bin"], markDownComments = false)
  files.["fslib-class.html"] |> shouldContainText "Readonly int property"
  files.["fslib-record.html"] |> shouldContainText "This is name"
  files.["fslib-record.html"] |> shouldContainText "Additional member"
  files.["fslib-union.html"] |> shouldContainText "Hello of int"
...
let ``ApiDocs highlights code snippets in Markdown comments``() =
  ...
  //ApiDocs.GenerateFromModelWithDotLiquid([library], output, layoutRoots, info, libDirs = [root </> "../../lib"], markDownComments = true)
