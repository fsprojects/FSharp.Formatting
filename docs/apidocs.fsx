...
`fsdocs` does not copy image files referenced from XML or Markdown `///` doc comments: it has no way to
know where an image such as `images/pi.png` lives relative to the source file, so it never walks the
source tree looking for assets.

To embed an image in an API doc comment, place it under the `docs` input directory (for example
`docs/content/images/pi.png`, which `fsdocs` copies to the output automatically) and reference it with a
site-root-relative path starting with `/`:

```fsharp
/// <summary>
/// <img src="/content/images/pi.png" alt="Pi diagram" />
/// </summary>
let pi = 3.14159
```

or, with Markdown comments (`<UsesMarkdownComments>true</UsesMarkdownComments>`):

```fsharp
/// ![Pi diagram](/content/images/pi.png)
let pi = 3.14159
```

The leading `/` matters: `{{fsdocs-root}}` and other template substitutions apply to template files, not
to the content of doc comments, so a relative path such as `images/pi.png` will not resolve correctly from
a nested API reference page.

For images produced by literate `.fsx` scripts under `docs`, see [`--saveimages`](commandline.html#embedding-images)
and [Embedding Images Generated by Scripts](commandline.html#embedding-images-generated-by-scripts) instead — that
pipeline resolves image paths relative to the script file.
