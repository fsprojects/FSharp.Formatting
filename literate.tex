...
\begin{itemize}
\item The \texttt{lang=foo} command specifies the language of the named snippet. If the language
...
as follows using functionality from the \href{./reference/fsharp-formatting-literate-literate.html}{Literate} type:
\begin{lstlisting}[numbers=left]
...
The methods used above (\href{./reference/fsharp-formatting-literate-literate.html\#ConvertScriptFile}{Literate.ConvertScriptFile}, \href{./reference/fsharp-formatting-literate-literate.html\#ConvertMarkdownFile}{Literate.ConvertMarkdownFile})
...
\subsection*{Using Literate scripts as doc tests}



Because Literate scripts execute embedded F\# code via F\# Interactive when the \texttt{eval} option
(or \texttt{fsdocs build --eval} / \texttt{fsdocs watch --eval}) is enabled, they double as a lightweight
form of "doc testing": the code samples in your documentation are run for real, and any
exceptions raised during evaluation cause the build to fail (or, with the \texttt{(*** include-output ***)}
directive, the \emph{actual} output of the snippet is captured and included verbatim in the
...
output against actual output), but combined with \texttt{(*** include-output ***)} and
\texttt{(*** include-value ***)} it gives a useful low-effort way to keep documentation examples
honest.
