...
\texttt{root} & The relative path from the page to the root of the site, \textbf{ending with \texttt{/} (\texttt{./}, \texttt{../}, ...)}, so write \texttt{\{\{root\}\}content/style.css} and not \texttt{\{\{root\}\}/content/style.css}. Links built with it work wherever the site is hosted: a domain root, a sub path, or the file system.\\ \hline
\texttt{fsdocs-site-root} & The absolute URL of the site, ending with \texttt{/}: \texttt{<PackageProjectUrl>} else \texttt{/} followed by \texttt{fsdocs-collection-name}. Only for the links that must be absolute, such as \texttt{og:url}, \texttt{twitter:site} and \texttt{llms.txt}; use \texttt{root} everywhere else. Override it with \texttt{--parameters fsdocs-site-root <url>} (\texttt{build}) or \texttt{--site-root <url>} (\texttt{watch}, where it defaults to the address of the server).\\ \hline
...
the text \texttt{`cref:T:FSharp.Formatting.Markdown.MarkdownParagraph`} resolves to the link \href{./reference/fsharp-formatting-markdown-markdownparagraph.html}{MarkdownParagraph}
