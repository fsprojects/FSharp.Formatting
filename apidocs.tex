...
in the \href{./reference/fsharp-formatting-apidocs-apidocs.html}{ApiDocs} type. To do this, load the assembly and open the necessary namespaces:
\begin{lstlisting}[numbers=left]
...
For example the \href{./reference/fsharp-formatting-apidocs-apidocs.html\#GenerateHtml}{ApiDocs.GenerateHtml} method:
\begin{lstlisting}[numbers=left]
...
The \texttt{root} parameter of the API is used for the base of page and image links in the generated documentation.


\texttt{fsdocs} generates relative links: each page gets a \texttt{root} that points from the page to the root of the
site (\texttt{./}, \texttt{../}, ...), so the output works from any host name, sub path or the file system.
The absolute URL of the site, derived from the project's \texttt{<PackageProjectUrl>} property, is available as
\texttt{fsdocs-site-root} for the links that must be absolute (Open Graph metadata, \texttt{llms.txt}); override it with
\texttt{--parameters fsdocs-site-root <url>}.
