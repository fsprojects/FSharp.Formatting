...
\texttt{(*** include-it ***)} or `(*** include-it-raw **
\begin{lstlisting}[numbers=left]

[escapeinside=\\\{\}]
\com{        // Convert the chart to a PNG byte array}
\com{        let bytes = chart.ToPngBytes()}
\com{        let b64 = System.Convert.ToBase64String(bytes)}
\com{        // Return (no extra resources, HTML string)}
\com{        Seq.empty, sprintf """<img src="data:image/png;base64,\%s" />""" b64)}

\com{    let myChart = MyChart.Create(data)}
\com{    (*** include-it ***)}

\com{With this printer registered, `(*** include-it ***)` will emit the chart as an inline Base64 image}
\com{in the HTML output.}

\com{If you don't have a custom type but still want to embed an image produced by your script, you can}
\com{use a plain helper function together with `(*** include-it-raw ***)`:}

\com{    [lang=fsharp]}
\com{    let inlinePng (fileName: string) =}
\com{        let bytes = System.IO.File.ReadAllBytes(fileName)}
\com{        let b64 = System.Convert.ToBase64String(bytes)}
\com{        sprintf """<img src="data:image/png;base64,\%s" />""" b64}
...
\com{The `(*** hide ***)` command suppresses the source code of the expression so that only the rendered}
\com{image appears in the output. See [Embedding Images](commandline.html\#embedding-images) in the command-line}
\com{reference for more details, including the `--saveimages` flag for downloading remote images.}

\com{\#\# Emitting Raw Text}

\com{To emit raw text in F\# literate scripts use the following:}
...
\com{which would emit}

\com{<pre>}
\com{Some raw text.}
\com{</pre>}

\com{directly into the document.}

\com{F\# Formatting as a Library:  Specifying the Evaluator and Formatting}
\com{---------------------------------------}

\com{If using F\# Formatting as a library the embedding of F\# output requires specifying an additional parameter to the}
\com{parsing functions discussed in [literate programming documentation](literate.html).}
\com{Assuming you have all the references in place, you can now create an instance of}
\com{`cref:T:FSharp.Formatting.Literate.Evaluation.FsiEvaluator` that represents a wrapper for F\# interactive and pass it to all the}
\com{functions that parse script files or process script files:}
...
\kwd{let} {fsiEvaluator} \ops{=} {FsiEvaluator}{(}{)}
