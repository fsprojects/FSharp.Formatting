...
\texttt{fsi.AddHtmlPrinter} lets you register a function that renders values of a particular type as raw HTML.
...
\texttt{(*** include-it ***)} or \texttt{(*** include-it-raw ***)}.


A common use-case is embedding chart or plot images. For example, if your charting library produces
values of type \texttt{MyChart}, you can register a printer that converts them to an \texttt{<img>} tag:
\begin{lstlisting}[numbers=left]

[escapeinside=\\\{\}]
\id{fsi}{.}\id{AddHtmlPrinter}{(}\kwd{fun} {(}\id{chart}{:} \id{MyChart}{)} \kwd{->}
    \com{// Convert the chart to a PNG byte array}
    \kwd{let} \id{bytes} \ops{=} \id{chart}{.}\id{ToPngBytes}{(}{)}
    \kwd{let} \id{b64} \ops{=} \id{System}{.}\id{Convert}{.}\id{ToBase64String}{(}\id{bytes}{)}
    \com{// Return (no extra resources, HTML string)}
    \id{Seq}{.}\id{empty}{,} \id{sprintf} \str{"""<img src="data:image/png;base64,\%s" />"""} \id{b64}{)}

\kwd{let} \id{myChart} \ops{=} \id{MyChart}{.}\id{Create}{(}\id{data}{)}
\com{(*** include-it ***)}
...
With this printer registered, \texttt{(*** include-it ***)} will emit the chart as an inline Base64 image
in the HTML output.


If you don't have a custom type but still want to embed an image produced by your script, you can
use a plain helper function together with \texttt{(*** include-it-raw ***)}:
\begin{lstlisting}[numbers=left]

[escapeinside=\\\{\}]
\kwd{let} \lfun{inlinePng} {(}\lfun{fileName}{:} \ltyp{string}{)} \ops{=}
    \kwd{let} \lfun{bytes} \ops{=} \id{System}{.}\id{IO}{.}\ltyp{File}{.}\id{ReadAllBytes}{(}\lfun{fileName}{)}
    \kwd{let} \lfun{b64} \ops{=} \id{System}{.}\ltyp{Convert}{.}\id{ToBase64String}{(}\lfun{bytes}{)}
    \lfun{sprintf} \str{"""<img src="data:image/png;base64,}\lprf{\%s}\str{" />"""} \lfun{b64}

\com{// Generate the image, then embed it:}
...
The \texttt{(*** hide ***)} command suppresses the source code of the expression so that only the rendered
image appears in the output. See \href{commandline.html\#embedding-images}{Embedding Images} in the command-line
reference for more details, including the \texttt{--saveimages} flag for downloading remote images.
...
\href{./reference/fsharp-formatting-literate-evaluation-fsievaluator.html}{FsiEvaluator} that represents a wrapper for F\# interactive and pass it to all the
...
\kwd{let} {fsi} \ops{=} \kwd{new} {FsiEvaluator}{(}{)}

\kwd{let} \id{doc} \ops{=} \ltyp{Literate}{.}\id{ParseScriptString}{(}\id{content}{,} \lfun{fsiEvaluator} \ops{=} {fsi}{)}
...
The constructor of \href{./reference/fsharp-formatting-literate-evaluation-fsievaluator.html}{FsiEvaluator} takes command line parameters for \texttt{fsi.exe} that can
...
This can be done by calling \href{./reference/fsharp-formatting-literate-evaluation-fsievaluator.html\#RegisterTransformation}{FsiEvaluator.RegisterTransformation} on the \texttt{FsiEvaluator} instance:
\begin{lstlisting}[numbers=left]

[escapeinside=\\\{\}]
\com{// Create evaluator \& register simple formatter for lists}
\kwd{let} {fsiEvaluator} \ops{=} \kwd{new} {FsiEvaluator}{(}{)}

{fsiEvaluator}{.}\id{RegisterTransformation}{(}\kwd{fun} {(}\lfun{o}{,} \lfun{ty}{,} \id{\_executionCount}{)} \kwd{->}
       ...
       \ops{\&\&} \lfun{ty}{.}\id{GetGenericTypeDefinition}{(}{)} \ops{=} \kwd{typedefof}{<}\ltyp{list}{<}\id{\_}{>}{>} \kwd{then}
        \kwd{let} \lfun{items} \ops{=}
            ...
            {[} \kwd{for} \lfun{it} \kwd{in} \ltyp{Seq}{.}\id{cast}{<}\ltyp{obj}{>} {(}\lfun{unbox} \lfun{o}{)} \kwd{->}
                  {[} {Paragraph}{(}{[} {Literal}{(}\lfun{it}{.}\id{ToString}{(}{)}{,} \ltyp{MarkdownRange}{.}\id{zero}{)} {]}{,} \ltyp{MarkdownRange}{.}\id{zero}{)} {]} {]}
        \com{// Return option value (success) with ordered list}
        {Some} {[} {ListBlock}{(}\ltyp{MarkdownListKind}{.}\id{Ordered}{,} \lfun{items}{,} \ltyp{MarkdownRange}{.}\id{zero}{)} {]}
    \kwd{else}
        {None}{)}
...
\kwd{let} \id{docOl} \ops{=} \ltyp{Literate}{.}\id{ParseScriptString}{(}\id{listy}{,} \lfun{fsiEvaluator} \ops{=} {fsiEvaluator}{)}
