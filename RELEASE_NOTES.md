...
* Display of type constraints (e.g. `'T : equality`, `'T : comparison`, `'T :> IComparable`) in generated API documentation. Constraints are shown inline as a `when` clause by default (e.g. `'T when 'T : equality`). Controlled by `<FsDocsTypeConstraints>` in the project file with values `None`, `Short` (default, inline), and `Full` (separate "Constraints:" section). [#591](https://github.com/fsprojects/FSharp.Formatting/issues/591)
