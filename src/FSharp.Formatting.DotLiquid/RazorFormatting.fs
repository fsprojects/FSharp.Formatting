...
/// generated by the F# or C# compiler using DotLiquid templates. To generate documentation, use one of the overloades of the `Generate` method.
...
type DotLiquidApiDocs private() =
  ...
  static let generate namespaceTemplate moduleTemplate typeTemplate layoutRoots outDir assemblyReferences (generatorOutput: ApiDocsModel) =
    ...
    let razor = DotLiquidRender<AssemblyGroup>(layoutRoots, ["FSharp.Formatting.ApiDocs"], namespaceTemplate, ?references = assemblyReferences)
    ...
    let razor = DotLiquidRender<ModuleInfo>(layoutRoots, ["FSharp.Formatting.ApiDocs"], moduleTemplate, ?references = assemblyReferences)
    ...
    let razor = new DotLiquidRender<TypeInfo>(layoutRoots, ["FSharp.Formatting.ApiDocs"], typeTemplate, ?references = assemblyReferences)
...
type DotLiquidLiterate private () =
  ...
  static let generateFile references contentTag parameters templateOpt output layoutRoots =
    ...
    | Some (file:string) when file.EndsWith("cshtml", true, System.Globalization.CultureInfo.InvariantCulture) ->
      let razor = DotLiquidRender(layoutRoots |> Seq.toList, [], file, ?references = references)
