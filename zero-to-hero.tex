...
🪄 When something looks off (a missing logo, a wrong title, a stale page), open \href{http://localhost:8901/.fsdocs/doctor}{\href{http://localhost:8901/.fsdocs/doctor}{http://localhost:8901/.fsdocs/doctor}}:
it shows what \texttt{fsdocs} found (projects, substitutions, templates, navigation) and why. See \href{commandline.html}{the command line docs} for details.
...
The links between the pages, and to the stylesheets, scripts and images, are relative: the \texttt{\{\{root\}\}} substitution used all over the default template
is the path from the page to the root of the site (\texttt{./}, \texttt{../}, ...). The output therefore works wherever it is hosted, and you can open
the files of the \texttt{output} folder directly in a browser to preview them.


\texttt{<PackageProjectUrl>} becomes the \texttt{\{\{fsdocs-site-root\}\}} substitution, the absolute URL of the site. It is only used where a link
must be absolute: the Open Graph metadata of the pages and \texttt{llms.txt}. Override it with \texttt{--parameters fsdocs-site-root <url>} when the site is published elsewhere.
